...
def mock_clip():
    ...
    with patch("app.pipeline.visual.CLIPModel.from_pretrained") as mock_model_cls, \
         patch("app.pipeline.visual.CLIPProcessor.from_pretrained") as mock_processor_cls:
        ...
        mock_model.return_value = make_mock_clip_outputs(
            VisualPipeline.DEFAULT_LABELS
        )
...
def test_return_visual_analysis_result(pipeline, test_frames):
    ...
    assert all(isinstance(analysis, FrameAnalysis) for analysis in result.frame_analyses)
