...
class VisualPipeline:
    ...
    def __init__(self, use_blip: bool = False):
        ...
        logger.info(f"VisualPipeline initialized with use_blip={self.use_blip} on device {self.device}")
    ...
    def _analyze_frame(
        ...
        caption = (
            ...
            if self.use_blip
            else max(clip_scores, key=clip_scores.get)  # Fallback to top CLIP label as caption
        )
    ...
    def _select_key_frames(
        ...
        scored = [
            (max(analysis.clip_scores.values()) if analysis.clip_scores else 0, analysis)
